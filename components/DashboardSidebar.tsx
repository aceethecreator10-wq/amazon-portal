...
const roleNav: Record<string, NavItem[]> = {
  ...
  admin: [
    ...
    { href: "/admin/payouts", label: "Payouts", icon: "P" },
    ...
    <aside className="w-64 shrink-0 bg-white border-r border-slate-200 min-h-screen hidden lg:flex flex-col">
      <div className="p-4 border-b border-slate-200">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-slate-900">DealFlow <span className="text-cyan-600">Portal</span></span>
        </Link>
      </div>
        ...
        {items.map((item) => (
          <Link key={item.href} href={item.href}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
              pathname === item.href ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
            }`}
          >
            <span className="w-6 h-6 rounded-md bg-slate-100 flex items-center justify-center text-xs font-bold">{item.icon}</span>
            {item.label}
          </Link>
        ))}
      </nav>
